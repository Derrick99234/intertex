...
const Shopping = ({ slug }: { slug: string }) => {
  ...
  for (const tab of tabData) {
    const prod = tab.products.find((p) => p.slug === slug);
    if (prod) {
      foundProduct = prod;
      currentCategory = tab;
      break;
    ...
    <div className="max-w-[1400px] mx-auto md:py-8 px-2 md:px-8">
      ...
      <div className="flex flex-col md:flex-row gap-10 bg-white rounded-xl">
        ...
        <div className="flex-1 flex justify-center flex-col md:gap-2 gap-1">
          ...
          <Accordion title="Offers on this product">{product.offers}</Accordion>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <ul className="list-disc ml-6 md:text-[19px] font-normal text-xs text-[#00041D] mb-8">
                ...
                >
                  <div>
                    ...
                    <div className="md:text-base text-xs text-[#152F24]">
                      {product.stock && typeof product.stock[size] === "number"
                        ? product.stock[size]
                        : 0}{" "}
                      products left
                    </div>
